...
router.post('/', rolesGestion, async (req, res) => {
  const { nombre, codigo_ica, municipio, departamento, direccion, area_hectareas, encargado_responsable, poligono_geojson, observaciones, latitud, longitud } = req.body;

  if (!nombre || !codigo_ica) {
    return res.status(400).json({ status: 'error', message: 'nombre y codigo_ica son obligatorios' });
  }

  if ((latitud != null && latitud !== '') && (Number.isNaN(Number(latitud)) || Number(latitud) < -90 || Number(latitud) > 90)) {
    return res.status(400).json({ status: 'error', message: 'latitud debe estar entre -90 y 90' });
  }
  if ((longitud != null && longitud !== '') && (Number.isNaN(Number(longitud)) || Number(longitud) < -180 || Number(longitud) > 180)) {
    return res.status(400).json({ status: 'error', message: 'longitud debe estar entre -180 y 180' });
  }
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO fincas
        (nombre, codigo_ica, municipio, departamento, direccion, area_hectareas, encargado_responsable, poligono_geojson, observaciones, latitud, longitud)
       VALUES (?, ?, ?, ?, ?, ?, ?, CAST(? AS JSON), ?, ?, ?)`,
      [
        nombre,
        codigo_ica,
        ...
        observaciones || null,
        latitud != null && latitud !== '' ? Number(latitud) : null,
        longitud != null && longitud !== '' ? Number(longitud) : null
      ]
    );
